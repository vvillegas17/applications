...
WIDTH = 79 # The width of cell grid.
...
# (!) Try changing ALIVE to '|' and DEAD to '_'.
...
for x in range(WIDTH): # Loop over every possible row.
    for y in range(HEIGHT): # Loop over every possible colum.
...
while True: # Main program loop.
    ...
    for x in range(WIDTH):
        for y in range(HEIGHT):
            ...
            # Count the number living neighbors:
            numNeighbors = 0
    ...
    try:
        time.sleep(1)  # Add a 1 second pause to produce flickering
